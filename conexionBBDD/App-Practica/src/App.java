...
//caracteres.
        ...
        try (Connection con = sqlData.getConnection()) {
            ...
            cargarFichero(con, "usuarios.csv");
...
// usuario añadir, llamamos al método obtenerÚltimoId.
    public static void cargarFichero (Connection con, String nombreFichero){
        String ruta = "D:\\Educa\\daw1_a\\conexionBBDD\\App-Practica\\" + nombreFichero;
        File fichero = new File(ruta);
        String[] lineas = new String[0];
        BufferedReader br = null;
        String insertar = "INSERT  INTO Usuarios (id, usarname, email, password, estado) VALUES (?, ?, ?, ?, ?);";
...
while (null != linea){
    lineas = linea.split(";");
int id = obtenerUltimoId(con) + 5;
String userName = lineas[0];
String email = lineas[1];
String contraseña = lineas[2];
String estado = lineas[3];
    try (PreparedStatement st = con.prepareStatement(insertar)) {
        st.setInt(1, id);
        for (int i = 0; i < lineas.length; i++) {
            st.setString((i + 1), lineas[i]);
        }
        st.executeUpdate();
        con.commit();
    } catch (SQLException e) {
